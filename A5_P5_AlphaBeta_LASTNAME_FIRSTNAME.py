...
def DG_Max_Value(divisors, alpha, beta):
    ...
    for action in divisors:
        utility, _, nodes_visited = DG_Min_Value(
            eliminate(action, divisors), alpha, beta
        )
...
def DG_Min_Value(divisors, alpha, beta):
    ...
    return_action = -1111111
    for action in divisors:
        utility, _, nodes_visited = DG_Max_Value(
            eliminate(action, divisors), alpha, beta
        )
...
def DG_Alpha_Beta(divisors):

    utility, action, numNodes = DG_Max_Value(divisors, -69_420, 69_420)
...
divisor_game_agent(n, name, turnNum, DG_Alpha_Beta)
